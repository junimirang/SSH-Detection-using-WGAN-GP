...
def loading_dataset(): ## After loading csv file, Pandas Data Frameset Generation ##
    ...
    df_fake = pd.read_csv('fake_data_ssh_filtered_20210311.csv', index_col=0)
    df_compare = pd.read_csv('df_compare_20210305_1normalized.csv', index_col=0)

    #X = min_max_scaler.fit_transform(df[["log_count_total_connect", "log_cs_byte", "log_transmit_speed_BPS","log_count_connect_IP", "log_avg_count_connect", "Business.time"]])
    X = df[["log_count_total_connect", "log_cs_byte", "log_transmit_speed_BPS","log_count_connect_IP", "log_avg_count_connect", "Business.time"]]
    #X = pd.DataFrame(X)
    #X.columns = ["log_count_total_connect", "log_cs_byte", "log_transmit_speed_BPS","log_count_connect_IP", "log_avg_count_connect", "Business.time"]
    X_fake = df_fake[["log_count_total_connect", "log_cs_byte", "log_transmit_speed_BPS","log_count_connect_IP", "log_avg_count_connect", "Business.time"]]
    X = pd.concat([X, X_fake])
    Y = df[["LABEL"]]
    Y = pd.concat([Y, df_fake[["LABEL"]]])
    Z = df[["log_time_taken"]]
    Z = pd.concat([Z,df_fake[["log_time_taken"]]])
    K = df[["no_url"]]
    K = pd.concat([K,df_fake[["no_url"]]])
    L = df[["log_ratio_trans_receive"]]
    L = pd.concat(([L, df_fake[["log_ratio_trans_receive"]]]))
    N = df[["Destination", "Destination Port", "no_url"]]
    N = pd.concat([N, df_fake[["Destination", "Destination Port", "no_url"]]])

    X_compare = min_max_scaler.fit_transform(df_compare[["log_count_total_connect", "log_cs_byte", "log_transmit_speed_BPS", "log_count_connect_IP", "log_avg_count_connect", "Business.time"]])
    Y_compare = df_compare[["LABEL"]]
    Z_compare = min_max_scaler.fit_transform(df_compare[["log_time_taken"]])
    K_compare = df_compare[["no_url"]]
    L_compare = min_max_scaler.fit_transform(df_compare[["log_ratio_trans_receive"]])
...
def PCA(X, K, L, Z, Y, N, K_compare, L_compare, Z_compare, Y_compare, N_compare):
    ...
    #result1 = pd.DataFrame(min_max_scaler.fit_transform(pd.DataFrame(projected_X1)))
    #result1.columns = ["PC"]
    #result1 = result1.reset_index(drop=0)
    #result1['No_url'] = K
    result1[['No_url','Ratio_trans_receive_(Normal)','Browse_time_(Normal)',"Label"]] = K, L, Z, Y
